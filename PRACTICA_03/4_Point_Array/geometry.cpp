...
PointArray::PointArray() {
    size = 0;
    points = new Point[0]; // Permite eliminar más tarde
}
...
PointArray::PointArray( const PointArray &other) {
	/*Cualquier código de la clase PointArray tiene acceso a
    variables privadas como el tamaño y los puntos*/
    size = other.size;
...
void PointArray::push_back( const Point &p) {
    resize(size + 1);
    points[size -1] = p;
    // También podría utilizar Insertar (tamaño, p); 
 }
